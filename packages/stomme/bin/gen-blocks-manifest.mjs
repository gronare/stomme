...
// passthrough — so the caller is blind BELOW the block level. This manifest ships
// the block field trees so the caller can flag block-level drift (a block with a
// renamed/removed field, an unknown `type`, or a missing required field). Fetched raw at
// gronare/stomme@<release-sha>, alongside schema-manifest.json. See the docs:
// stomme-blocks-manifest.
//
// It is a REPORTING artifact (a lenient, faithful projection of the catalog), not a
// build-time gate — a deployment model tolerates/reports drift rather than hard-failing.
//
...
// Keeps only what the caller needs to diff frontmatter; UI-only keys (label, hint,
